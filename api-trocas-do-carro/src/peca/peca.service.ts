...
export class PecaService {
  ...
  async exibePecas(): Promise<(PecaEntity & { km_restante: string })[]> {
    let pecas = await this.pecaRepository.find({
      order: {
        id: 'ASC',
      },
    });
    ...
    for (const peca of pecas) {
      const kmRestante = String((Number(peca.km_troca) + Number(peca.km_proxima_troca)) - Number(km.quantidade));
  ...
  async exibePecaPorId(id: number): Promise<PecaEntity & { km_restante: string }> {
